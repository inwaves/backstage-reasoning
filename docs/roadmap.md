...
action-continuation signals indicate a risky trajectory before the visible
reasoning makes that risk legible to a text-only monitor.
...
5. **Three timing signals**
   ...
   - hidden-state probe prediction of risky continuation;
   - forced next action or short forced rollout from environment checkpoints or
     visible prefixes;
   - text-only monitor prediction from visible reasoning prefixes.
